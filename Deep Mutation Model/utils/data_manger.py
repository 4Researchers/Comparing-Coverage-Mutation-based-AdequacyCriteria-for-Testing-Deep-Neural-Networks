...
def load_data_set(data_type, source_data, train=False):
    if data_type == DATA_MNIST:
        data = torchvision.datasets.MNIST(root=source_data, train=train, download=True, transform=torchvision.transforms.Compose(
            [
        ...
        data = torchvision.datasets.CIFAR10(root=source_data, train=train, download=True,
...
def load_imagenet():
    ...
    for data, lable in loader:
        ...
        imageio.imwrite('img_' + str(count) + '_l_' + str(lable.item()) + '.JPEG', img)
        count += 1
    ...
    for img_file in os.listdir(adv_file_path):
        if img_file.endswith('.png'):
            ...
            if filter:
                ...
                img = imageio.imread(adv_file_path + os.sep + img_file)
                if channels == 1:
...
class MyDataset(torch.utils.data.Dataset):
    ...
    def __init__(self, root, transform=None, target_transform=None, img_mode=None, show_extral_label=True,
                 show_file_name=False, max_size=10000):          #This might be the problem!!!!!!!!!!!!!!!!!
        ...
        for img_file in load_files:
            if img_file.endswith('.png'):
                ...
                real_labels.append(real_label)
                predicted_labels.append(predicted_label)
                img = imageio.imread(root + os.sep + img_file)
                image_list.append(img)
