...
def samples_filter(model, loader, name, return_type="adv", size=0, show_progress=False,      # if the predict label is equal to the target label,then remove
    ...
    total_sample =len(loader.dataset)           #adversarial samples
    #print("len of original dataset")
    #print(total_sample)
    #print("\n")
    model = model.to(device)
    for data_tuple in loader:
        ...
        if is_verbose:
            if len(data_tuple) == 4:
                ...
                logging.info('>>>True Label:{},adv_label:{}'.format(target.item(), adv_label.item()))              #The original label
            else:
                logging.info('>>>True Label:{}'.format(target.item()))                 #used in normal testing
        output = model(data)
        test_loss += F.nll_loss(output, target, size_average=False).item()  # sum up batch loss
        pred = output.data.max(1, keepdim=True)[1]  # get the index of the max log-probability
        if is_verbose:
            logging.info('Predicted Label:{}<<<'.format(pred.item()))
        rst = pred.eq(target).sum().item()  #0 or 1
        correct += rst
